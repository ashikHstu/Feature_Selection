...
def GeneticAlgorithm(Data,class_name='class',number_of_population=10,generation=3,crossover_probability=1,mutation_probability=0.2):
    print("Default Running...")
    Xold = Data.drop(class_name, axis=1)
    Y = Data[class_name]
    ...
    p_c = crossover_probability# prob. of crossover
    p_m = mutation_probability# prob. of mutation
    pop = number_of_population
    gen = generation
    kfold = 5
    ...
    for i in range(gen):
        ...
        if len(Not_So_Darwin_Guy) == 0:
            print("Not_So_Darwin_Guy is empty!")
              # Check the logic that generates Not_So_Darwin_Guy

        print("New_Population shape:", New_Population.shape)
        #print("Darwin_Guy shape:", Darwin_Guy.shape)
        #print("Not_So_Darwin_Guy shape:", Not_So_Darwin_Guy.shape)
        print("new population : ",New_Population)
        print("not so darwin : ",Not_So_Darwin_Guy)
        print("Darwin : ",Darwin_Guy)
    ...
    finalCromosome=One_Final_Guy_Final[:,2:]
    finalAcc=1-One_Final_Guy_Final[:,1]
    ...
    return emp_list,finalCromosome,finalAcc


def GeneticAlgorithm_singleCromosomePass(Data,Cromosome,class_name='class',number_of_population=10,generation=3,crossover_probability=1,mutation_probability=0.2):
    print("Single Cromo running...")
    Xold = Data.drop(class_name, axis=1)
    Y = Data[class_name]
    norm = preprocessing.MinMaxScaler()
    X = norm.fit_transform(Xold)
...
def GeneticAlgo(Data,singleCromosome=None,cromosomeSet=None,class_name='class'):
    print("single cromo : ");
    print(singleCromosome)
    if singleCromosome!=None:
        print("single cromo running")
        finalFeature,finalCromo,res=GeneticAlgorithm_singleCromosomePass(Data.copy(),Cromosome=singleCromosome,class_name='class')
    else:
        finalFeature,finalCromo,res=GeneticAlgorithm(Data.copy(),class_name='class')
        
    allF=Data.columns.tolist()
    newFinalFeature = [allF[i] for i in finalFeature]
    newFinalFeature.append('class')
    newData=Data[newFinalFeature]
    res=quick_result(newData.copy(),class_name="class",test_size=0.20,random_state=42)
    finalResult=myResult(res,finalFeature,finalCromo,newData)
    return finalResult
